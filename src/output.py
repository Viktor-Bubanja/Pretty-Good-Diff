...
def output(diff, sentinel = object()):
    print(f"\n{_blue_output('actual')} vs {_purple_output('expected')}\n\n")
    if type(diff) == dict:
        output_dict_diff(diff, sentinel)
    if type(diff) == list:
        output_str_diff(diff, sentinel)


def output_dict_diff(output_dict: dict, sentinel = object(), indent = 0) -> None:
    missing_message = _red_output("<missing>")

    for key, value in output_dict.items():
        if type(value) == dict:
            print(indent * " " + _yellow_output(key))
            output_dict_diff(value, sentinel, (indent + INDENT_SIZE))
        else:
            first_obj, second_obj = value
            actual_output = missing_message if first_obj == sentinel else _blue_output(first_obj)
            expected_output = missing_message if second_obj == sentinel else _purple_output(second_obj)

            out_string = "{}: {} {}".format(_yellow_output(key), actual_output, expected_output)
            print(indent * " " + out_string)


def output_str_diff(substrings: list, sentinel = object()) -> None:
    diff = ""
