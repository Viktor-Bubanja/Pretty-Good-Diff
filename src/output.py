    ...
    for key, value in output_dict.items():
        if type(value) == dict:
            ...
            if matching_substrings:
                ...
                actual_output = missing_message if first_obj is sentinel else _blue_background_output(first_obj)
                expected_output = missing_message if second_obj is sentinel else _purple_background_output(second_obj)
...
def output_str_diff(first_object, second_object, substrings: list) -> None:
    diff = ""
    first_obj_mappings, second_object_mappings = [sub[0] for sub in substrings], [sub[1] for sub in substrings]
    ...
    for x, y in zip(first_obj_mappings, second_object_mappings):
        if x[0] > previous_first_index:
            diff += _blue_background_output(first_object[slice(previous_first_index+1, x[0])])
        if y[0] > previous_second_index:
            diff += _purple_background_output(second_object[slice(previous_second_index+1, y[0])])
